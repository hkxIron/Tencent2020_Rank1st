...
class ctrNet(nn.Module):
    def __init__(self, args):
        ...
        classify_model = ClassifyModel(args)
        classify_model.to(args.device)
    ...
    def train(self, train_dataset, dev_dataset=None):
        ...
        # 设置优化器
        optimizer = AdamW(self.classify_model.parameters(), lr=args.lr, eps=1e-8, weight_decay=0.08)
        scheduler = get_linear_schedule_with_warmup(optimizer,
                                                    num_warmup_steps=int(len(train_dataloader) * args.num_train_epochs * 0.2),
                                                    num_training_steps=int(len(train_dataloader) * args.num_train_epochs))
        # 多GPU设置
        if args.n_gpu > 1:
                ...
                if (step + 1) % args.eval_steps == 0 and dev_dataset is not None:
                    ...
                    if results['eval_age_acc'] > best_age_acc:
                        ...
                        try:
                            ...
                            pass
                        model_to_save = classify_model.module if hasattr(classify_model,
                                                                         'module') else classify_model  # Only save the ctr_model it-self
